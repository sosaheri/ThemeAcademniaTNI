...
$output .= '<div class="row clearfix gs_wpposts">';
    ...
    if ( $GLOBALS['gs_wpposts_loop']->have_posts() ) {
        while ( $GLOBALS['gs_wpposts_loop']->have_posts() ) {
            ...
            $gswppost_post_tag       = get_the_tag_list('<span> ',', ','</span>');
            ...
            $gswppost_wppost_details            = (strlen($gswppost_wppost_details) > 50) ? substr($gswppost_wppost_details,0, $desc_limit ).'...<a href="'.$gswppost_wppost_details_link.'" target="_self">'.$gswppost_op_wppost_details_readmore.'</a>' : $gswppost_wppost_details;

            $output .= '<div class="col-md-'.$cols.' col-sm-6 col-xs-6">';
                $output .= '<div class="single-wppost">'; // start single wppost
                    if ( has_post_thumbnail() )
                        $output .= '<img src="'. $gswppost_wppost_thumb .'" alt="'. $gswppost_wppost_alt .'" />';
                    else {
                    ...
                    $output .= '<div class="single-wppost-overlayer">'; // start overlayer
                        if ( !empty( $gswppost_wppost_details ) && 'on' ==  $gswppost_op_wppost_details ) :
                            $output.= '<div class="gs-wppost-details">'. $gswppost_wppost_details .'</div>';
                        endif;

                        if ( !empty( $gswppost_post_tag ) && 'on' ==  $gswppost_op_tag ) :
                            $output.= '<div class="gs-wppost-tag"><span class="dashicons dashicons-tag"></span>'. $gswppost_post_tag .'</div>';
                        endif;
